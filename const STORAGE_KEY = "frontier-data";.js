...
function switchView(view) {
  currentView = view;
  ["home", "article", "submit", "editor"].forEach((v) => {
    el(`view-${v}`).style.display = v === view ? "" : "none";
  });
  ["Home", "Submit", "Editor"].forEach((label) => {
    const btn = el(`nav${label}`);
    const btnView = label.toLowerCase();
    btn.classList.toggle("active", view === btnView);
  });
...
function renderHome() {
  ...
  if (articles.length === 0) {
    grid.innerHTML = `<p class="empty-msg">No dispatches published yet. Check back soon.</p>`;
    return;
  }
...
function resetSubmitForm() {
  ["p-title", "p-pitch", "p-name", "p-email"].forEach((id) => (el(id).value = ""));
  ["err-p-title", "err-p-pitch", "err-p-name"].forEach((id) => (el(id).textContent = ""));
...
function handleSendPitch() {
  ...
  el("thanksText").textContent =
    `Thanks, ${name}. Your pitch is with the editors now — they'll be in touch if they want to take it further.`;
...
function renderEditor() {
  ...
  el("pendingCount").textContent = `Pending Pitches (${pending.length})`;
  const pendingList = el("pendingList");
  if (pending.length === 0) {
    pendingList.innerHTML = `<p class="empty-msg">No pitches waiting on review.</p>`;
  } else {
    pendingList.innerHTML = pending
      .map(
        ...
        <div class="card editor-card">
          ...
          <p class="card-meta">${escapeHtml(s.name)}${s.email ? " · " + escapeHtml(s.email) : ""}</p>
...
function handlePublish() {
  ...
  ["e-title", "e-author", "e-issue", "e-excerpt", "e-content"].forEach((id) => (el(id).value = ""));
  renderHome();
